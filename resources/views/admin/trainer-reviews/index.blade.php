...
@section('title', 'Отзывы о тренерах')

@push('styles')
<link rel="stylesheet" href="{{ asset('css/admin/trainer-reviews/index.css') }}">
@endpush
...
<div class="admin-container">
    <div class="page-header">
        ...
        <h1>Отзывы о тренерах</h1>
    </div>

    <div class="review-filter-row">
        <a href="{{ route('admin.trainer-reviews.index', ['status' => 'pending']) }}" class="review-filter-chip {{ $status === 'pending' ? 'is-active' : '' }}">
            На модерации <span>{{ $stats['pending'] }}</span>
        </a>
        <a href="{{ route('admin.trainer-reviews.index', ['status' => 'approved']) }}" class="review-filter-chip {{ $status === 'approved' ? 'is-active' : '' }}">
            Одобрены <span>{{ $stats['approved'] }}</span>
        </a>
        <a href="{{ route('admin.trainer-reviews.index', ['status' => 'rejected']) }}" class="review-filter-chip {{ $status === 'rejected' ? 'is-active' : '' }}">
            Отклонены <span>{{ $stats['rejected'] }}</span>
        </a>
        <a href="{{ route('admin.trainer-reviews.index', ['status' => 'all']) }}" class="review-filter-chip {{ $status === 'all' ? 'is-active' : '' }}">
            ...
            <article class="review-admin-card">
                <div class="review-admin-head">
                    <div class="review-admin-main">
                        <div class="review-admin-trainer">{{ $review->trainer?->name ?? 'Тренер удален' }}</div>
                ...
                <form action="{{ route('admin.trainer-reviews.update', ['id' => $review->id, 'status' => $status]) }}" method="POST" class="review-admin-form">
                    @csrf
                    @method('PUT')
